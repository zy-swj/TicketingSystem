...
<head>
    <script>
        ...
        function endorse(seat_types,price){
            ...
            $.ajax({
                url:"${pageContext.request.contextPath}/order/endorseTickets.action",
                type:'post',
                data: order,
        ...
        <c:if test="${not empty _PRICE_}">
            <c:forEach items="${_PRICE_}" var="p" >
                <tr>
                    ...
                    <td></td>
                    <td><button type="button" class="btn btn-success" onclick="endorse('${p.key}','${p.value}');">确认改签</button></td>
                </tr>
            </c:forEach>
        </c:if>
        </tbody>
